...
const mockChats = [
  {
    id: 1,
    title: '카카오뱅크 앱 설치 도움',
    partner: userType => userType === 'user' ? '김전문가' : '박고객',
    partnerRating: 4.9,
  ...
  },
  {
    ...
    partner: userType => userType === 'user' ? '이도우미' : '최사용자',
    partnerRating: 5.0,
  ...
  {
    id: 3,
    title: '건강보험공단 건강검진 예약',
    partner: userType => userType === 'user' ? '정전문가' : '김시민',
    partnerRating: 4.8,
    lastMessage: '건강검진 예약 방법 알려드릴게요',
    timestamp: '1시간 전',
    unread: 1,
    online: true,
    status: 'progress'
...
export function ChatTab({ userType }: ChatTabProps) {
  ...
  const chatsToShow = mockChats.map(chat => ({
    ...chat,
    partner: chat.partner(userType)
  }));
      ...
      <div className="p-6 space-y-6">
        ...
        {filteredChats.length > 0 ? (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800">활성 채팅</h3>
